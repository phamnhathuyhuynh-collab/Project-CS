...
const animate = () =>{
    ...
    if(keys.leftKey.pressed && player.position.x > proportionalSize(100)){
        player.velocity.x = -5;
    }else if(keys.rightKey.pressed && player.position.x < proportionalSize(400)){
        player.velocity.x = 5;
    }else{
        player.velocity.x = 0;
    };

    if(keys.leftKey.pressed && isCheckpointCollisionDetectionActive){
        platforms.forEach((platform) => {platform.position.x += 5});
        checkpoints.forEach((checkpoint) => {checkpoint.position.x += 5});
    }else if (keys.rightKey.pressed && isCheckpointCollisionDetectionActive){
        platforms.forEach((platform) =>{platform.position.x -= 5});
        checkpoints.forEach((checkpoint) => {checkpoint.position.x -= 5});
    };
